...
export function analyzeCapturedVariables(funcExpr, outerLocals, functions) {
    ...
    for (const name in referenced) {
        ...
        console.log(`DEBUG: Checking if ${name} should be captured (outerLocals exists: ${!!outerLocals}, exists in outerLocals: ${outerLocals ? outerLocals[name] : "N/A"})`);
        // 检查是否在外部作用域中
...
export function collectLocalDeclarations(node, vars) {
    if (!node) return;
    console.log(`DEBUG: collectLocalDeclarations node.type=${node.type}`);

    if (node.type === "VariableDeclaration") {
        const decls = node.declarations || [];
        for (let i = 0; i < decls.length; i++) {
            if (decls[i].id) {
                console.log(`DEBUG: VariableDeclarator id.type=${decls[i].id.type} name=${decls[i].id.name}`);
        ...
        if (node.declaration) {
            console.log(`DEBUG: ExportDeclaration has declaration.type=${node.declaration.type}`);
...
export function analyzeTopLevelSharedVariables(ast) {
    console.log(`DEBUG: analyzeTopLevelSharedVariables starting for AST with ${ast?.body?.length || 0} statements`);
    const sharedVars = new Set();
    ...
    console.log(`DEBUG: mainLocalVars: ${Object.keys(mainLocalVars).join(", ")}`);
    ...
    for (const stmt of ast.body) {
        ...
        if (decl && (decl.type === "FunctionDeclaration" || decl.type === "ClassDeclaration")) {
            ...
            if (captured.length > 0) {
                console.log(`DEBUG: Top-level function ${decl.id ? decl.id.name : "anonymous"} captures: ${captured.join(", ")}`);
            }
            for (const name of captured) {
                sharedVars.add(name);
            }
