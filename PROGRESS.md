...
| selfhost 自举运行时崩溃 | cli-self 编译 helloworld | P0 | ⚠️ 运行时生成阶段崩溃：MathGenerator generateMathSqrt，正在修复 |
...
| ~~P1~~ | ~~`this.method(a, b)` 调用~~ | ~~多处内部方法调用~~ | ✅ **已修复** - 两参数 this.method() 调用被 Set.add() 错误拦截 |
| ~~P1~~ | ~~`obj.add()` 调用~~ | ~~vm.add() 等~~ | ✅ **已修复** - unknown 类型回退逻辑移除了冲突的方法名 |
| P1 | 自举运行时崩溃 | cli-self 编译 helloworld | 编译器内部代码自举后执行时 SIGSEGV，调试中 |
...
**2026-02-07 - this.method(a, b) 两参数调用修复**
- 问题：`this.add(100, 200)` 形式的两参数内部方法调用崩溃
- 根因：当 `objType === "unknown"` 时，`add` 方法名被 Set.add() 匹配，Set.add() 只消费一个参数
- 修复：在 functions.js 第 1724 行添加检查 `obj.type !== "ThisExpression"`
- 影响：修复了类内部方法的两参数调用

**2026-02-07 - obj.add() 等方法调用修复**
- 问题：`vm.add(dest, a, b)` 形式的三参数对象方法调用崩溃
- 根因：unknown 类型回退逻辑中的常见方法名（add, set, get, has 等）与用户定义方法冲突
- 修复：从 unknown 类型回退列表中移除冲突的方法名（add, set, get, indexOf, slice, replace 等）
- 只保留不太可能与用户方法冲突的安全方法名：toUpperCase, toLowerCase, charAt, trim 等
- 影响：修复了 vm.add、map.set 等对象方法调用

**2026-02-07 - UNKNOWN 类型 + 运算符修复**
- 问题：`return dest + a + b;` 字符串连接在方法内崩溃 (SIGSEGV)
- 根因：当操作数类型为 UNKNOWN 时（如函数参数），+ 运算符假设它们是 Number 对象指针
- 详情：直接从指针+8 加载 float64，但实际传入的是 NaN-boxed 字符串 (0x7ffc...)
- 修复：在 `compiler/expressions/operators.js` 的 `compileBinaryExpression` 中添加 UNKNOWN 类型检查
- 方案：对 UNKNOWN 类型的 + 运算调用运行时函数 `_js_add` 进行动态类型检测
- 同时：在 `runtime/index.js` 中注册 `AddGenerator`
- 影响：修复了 vm_add_test.js，所有带参数字符串连接的场景

**2026-02-08 - 运行时/词法修复与 MathGenerator 清理**
- 修复：`_js_add` 字符串识别改为 heap/data 段判断，统一使用 `_to_number` 做数值加法
- 修复：字符串 slice/substring 参数由 `_to_number` 转换，避免 Nan-boxing 误判
- 修复：lexer readChar 位置递增与标识符/nextToken 崩溃
- 修复：print 浮点尾数掩码常量改为数值字面量，避免 movImm64 字符串路径
- 修复：ARM64 label 前缀判断改为 `indexOf("_")`
- 调整：MathGenerator 内浮点指令改用 VM helper（`fmovToFloat`/`fsqrt`/`fmovToInt` 等），用于规避自举生成崩溃（待验证）

**2026-02-08 - 自举运行时推进 (Array/Error/ChildProcess)**
- 调整：Array.from 目前仅支持数组输入，字符串路径暂时跳过以避开自举生成崩溃（待恢复）
- 修复：`_error_to_string` 使用寄存器保存 name/message，避免栈 push/pop 路径导致崩溃
- 调整：`execSync` 运行时简化为返回空字符串，暂时绕过 syscalls
- 追踪：自举运行时生成已通过 ErrorGenerator，当前崩溃点推进到 BufferGenerator（DEBUG_RUNTIME）
...
#### 自举调试 (2026-02-07)
1. **修复 `_print_number` 处理原始 float64 值**:
   - 问题: `console.log(Infinity)` 和 `console.log(-Infinity)` 导致 segfault
   - 原因: 编译器将 `Infinity`/`-Infinity` 编译为原始 IEEE 754 位模式 (如 `0xfff0000000000000`)，但 `_print_number` 假设输入是堆指针并尝试解引用
   - 解决: 修改 `runtime/types/number/print.js` 中的 `generatePrintNumber` 和 `generatePrintNumberNoNL`，先检测输入是否在堆范围内，如果不是则直接作为原始 float64 打印
   - 修复位置: `runtime/types/number/print.js` 第 565-708 行

2. **测试结果改善**:
   - 之前: 201 passed, 18 failed
   - 现在: **205 passed, 14 failed**
   - 修复的测试: test_inf2.js, test_inf3.js, test_special_floats.js, test_special_numbers.js

3. **剩余失败测试 (14个)**:
   - parse_obj_test.js: 运行失败
   - test_bigint_tostring.js: segfault
   - test_exec_sync.js: 运行失败
   - test_fs_read.js: 运行失败
   - test_fs_stat2.js: 运行失败
   - test_fs_stat3.js: 运行失败
   - test_mask.js: segfault
   - test_padstart.js: segfault
   - test_path_getcwd.js: 运行失败
   - test_path_resolve.js: 运行失败
   - test_path_resolve2.js: 运行失败
   - test_shebang2.js: segfault
   - test_tostring.js: segfault
   - vm_add_test.js: segfault

   #### 自举调试 (2026-02-08)
   1. **修复 `_js_add` 误判字符串导致数字加法变为拼接**:
       - 现象: selfhost 运行时 `readPosition + 1` 变成字符串拼接，`readChar` 不前进。
       - 原因: `_js_add` 用高 32 位==1 判定数据段字符串，误将堆指针当作字符串。
       - 解决: 改为使用堆范围与 `_data_start` 范围检测，仅在堆字符串或数据段字符串时走拼接。
       - 影响: 数值 + 运算恢复为数值路径，避免 lexer/parser 崩溃。