...
export class PrintGenerator {
    ...
    generatePrintValue() {
        ...
        // ============ 核心检测逻辑 ============
        // 检查是否是标准 NaN (0x7ff8000000000000)
        vm.movImm64(VReg.V1, 0x7ff8000000000000n);
        vm.cmp(VReg.S0, VReg.V1);
        const notExactNaN = "_print_value_not_exact_nan_pf";
        vm.jne(notExactNaN);
        vm.lea(VReg.A0, "_str_nan");
        vm.call("_print_str");
        vm.jmp(doneLabel);

        vm.label(notExactNaN);
    ...
    generatePrintArray() {
        ...
        vm.load(VReg.A0, VReg.V0, 24); // 跳过 header (24 bytes)
    ...
    generatePrintArrayElemNoNL() {
        ...
        // 原始float: 使用 fcvtzs 转换为整数，然后打印
        vm.label(isRawFloatLabel);
        vm.fmovToFloat(0, VReg.S0);  // 转换为浮点值
        vm.fcvtzs(VReg.A0, 0);       // 转换为整数
