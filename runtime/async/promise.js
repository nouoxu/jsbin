...
export class PromiseGenerator {
    ...
    generatePromiseAll() {
        ...
        vm.mov(VReg.S0, VReg.A0); // input iterable

        // 创建空数组
        // 分配: 24 (header) + 8*8 (初始容量) = 88 bytes
        vm.movImm(VReg.A0, 24 + 64);
        vm.call("_alloc");
        vm.mov(VReg.S1, VReg.RET);

        // 设置数组头: type=1, length=0, capacity=8
        vm.movImm(VReg.V1, 1); // TYPE_ARRAY
        vm.store(VReg.S1, 0, VReg.V1);
        vm.movImm(VReg.V1, 0); // length = 0
        vm.store(VReg.S1, 8, VReg.V1);
        vm.movImm(VReg.V1, 8); // capacity = 8
    ...
    // Promise.race(iterable) - 静态方法
    // 返回任意一个 Promise 完成的结果
    // 简化实现：返回 undefined
    generatePromiseRace() {
        const vm = this.vm;
    ...
    // Promise.allSettled(iterable) - 静态方法
    // 等待所有 Promise settled，返回结果数组
    // 简化实现：返回空数组
