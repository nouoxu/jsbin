    ...
    compileArrayMethod(arrayExpr, method, args) {
        ...
        if (method === "push") {
            if (args.length > 0) {
                ...
                this.vm.mov(VReg.A0, VReg.RET);
                this.vm.movImm64(VReg.V4, 0x0000ffffffffffffn);
                this.vm.and(VReg.A0, VReg.A0, VReg.V4); // unbox
    ...
    compileStringMethod(obj, method, args) {
        ...
        switch (method) {
            ...
            case "charCodeAt":
                ...
                // RET contains raw char code (0-255) or -1 for out-of-bounds
                // Save the char code first
                this.vm.mov(VReg.S0, VReg.RET);
                // Check if -1 (out of bounds)
                const charCodeOOBLabel = this.ctx.newLabel("charCodeAt_oob");
                const charCodeEndLabel = this.ctx.newLabel("charCodeAt_end");
                this.vm.cmpImm(VReg.S0, -1);
                this.vm.jeq(charCodeOOBLabel);
                // Valid char code - box as integer
                this.boxIntAsNumber(VReg.S0);
                this.vm.jmp(charCodeEndLabel);
                // Out of bounds - box as NaN
                this.vm.label(charCodeOOBLabel);
                // Load NaN float64 bits
                this.vm.movImm64(VReg.S0, 0x7FF8000000000000n);
                // Allocate and box as float
                ...
                // RET stays as block pointer (no addImm) - _print_value expects block pointer
                this.vm.label(charCodeEndLabel);
                return true;
