...
export const ExpressionCompiler = {
    ...
    compileUserClassNew(className, args) {
        ...
        if (offset !== undefined || globalLabel !== undefined) {
            if (offset !== undefined) {
                // 类在局部变量中，加载类信息对象
            ...
            this.vm.call(this.getFunctionLabel(className));
    ...
    compileDynamicNew(constructorReg, args) {
        ...
        this.vm.movImm(VReg.V0, 2); // TYPE_OBJECT
        this.vm.store(VReg.S0, 0, VReg.V0);

        // 2. 设置 prototype (假设 JSBin 类结构：+16 是 prototype)
        this.vm.load(VReg.V0, constructorReg, 16); 
        this.vm.store(VReg.S0, 24, VReg.V0);

        // 3. 获取构造函数指针 (+8 是 constructor 指针)
        this.vm.load(VReg.S2, constructorReg, 8);

        // 4. 准备参数
        for (let i = 0; i < args.length && i < 7; i++) {
            this.vm.push(VReg.S0);
            this.vm.push(constructorReg);
            this.vm.push(VReg.S2);
            this.compileExpression(args[i]);
            this.vm.pop(VReg.S2);
            this.vm.pop(constructorReg);
            this.vm.pop(VReg.S0);
            const argReg = VReg.A0 + i + 1;
            if (argReg <= VReg.A7) {
                this.vm.mov(argReg, VReg.RET);
