...
export class VirtualMachine {
    ...
    // 调用函数
    call(label) {
        if (label === "_str_to_num" || label === "_abstract_eq") {
            console.log(`DEBUG vm.call: label=${label} backend.code.length=${this.backend.code?.length}`);
        }
    ...
    label(name) {
        if (name === "_str_to_num" || name === "_abstract_eq") {
            console.log(`DEBUG vm.label: name=${name} code.length=${this.backend.code?.length}`);
        }
    ...
    // 浮点取反: fpDest = -fpSrc
    fneg(fpDest, fpSrc) {
        this._emit(OpCode.FNEG, [fpDest, fpSrc]);
